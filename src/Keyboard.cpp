...
u64 CKeyboard::ReadMem(int index, u64 address, int dsize)
{
	...
	case 1:   return read_64(); break;
	default:  FAILURE(InvalidArgument, "kbc: ReadMem index out of range");
	}
}
...
#ifdef DEBUG_KBD
	BX_DEBUG(("enQ: putting scancode 0x%02x in internal buffer", (unsigned)scancode));
#endif
	tail =
		(
...
void CKeyboard::ctrl_to_mouse(u8 value)
{
	...
	if (state.expecting_mouse_parameter)
	{
		...
		case 0xe8:  // Set Mouse Resolution
			switch (value)
			{
			case 0:   state.mouse.resolution_cpmm = 1; break;
			case 1:   state.mouse.resolution_cpmm = 2; break;
			case 2:   state.mouse.resolution_cpmm = 4; break;
			case 3:   state.mouse.resolution_cpmm = 8; break;
			default:  BX_PANIC(("[mouse] Unknown resolution %d", value)); break;
			}

#ifdef DEBUG_KBD
			BX_DEBUG(("[mouse] Resolution set to %d counts per mm", state.mouse.
				resolution_cpmm));
#endif
			controller_enQ(0xFA, 1);  // ack
			break;
